...
from dataset.dataset_utils import IMG_SIZE, HAND_BBOX_RATIO, cut_img
from utils.vis_utils import mano_two_hands_renderer, mano_renderer

DROP_VIEW = ['400006', '400008', '400015', '400016', '400029', '400030', '400031', '400035', '400049']
...
mano_path = {'left': './misc/mano/MANO_LEFT.pkl',
            'right': './misc/mano/MANO_RIGHT.pkl'}
...
def cut_inter_img(loader, save_path, split):
    ...
    target = 'interacting' if opt.hand_type == 'both' else 'single'
    renderer = mano_two_hands_renderer(mano_path=mano_path, img_size=IMG_SIZE, device='cuda')
    if target == 'single': 
        renderer = {'left':  mano_renderer(hand_type='left', img_size=IMG_SIZE, device='cuda'),
                    'right': mano_renderer(hand_type='right', img_size=IMG_SIZE, device='cuda')}

    idx = 0
    for i in tqdm(range(len(loader))):
        annotation = loader.data_info['annotations'][i]
        images_info = loader.data_info['images'][i]
        capture_id = images_info['capture']

        img_name = images_info['file_name'].split('/')[-1].split('.')[0]
        hand_type = annotation['hand_type']
        hand_type_valid = annotation['hand_type_valid']
        
        if hand_type == target and hand_type_valid:
            mano_dict, coord_dict = loader.load_mano(i)
            if coord_dict['left'] is not None and coord_dict['right'] is not None:
                cam_id = images_info['camera']
                if cam_id.startswith('41') or cam_id in DROP_VIEW:
                    continue

                os.makedirs(osp.join(save_path, split, 'img', str(capture_id), cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'mask', str(capture_id), cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'masked_img', str(capture_id), cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'anno', str(capture_id), cam_id), exist_ok=True)
                ...
                if dist < 9999999:
                    ...
                    cv.imwrite(osp.join(save_path, split, 'img', str(capture_id), cam_id, '{}.jpg'.format(img_name)), img)
                    ...
                    with open(osp.join(save_path, split, 'anno', str(capture_id), cam_id, '{}.pkl'.format(img_name)), 'wb') as file:
                        pickle.dump(data_info, file)

                    idx = idx + 1

                    R = data_info['camera']['R']
                    T = data_info['camera']['t']
                    camera = data_info['camera']['camera']

                    verts = []
                    for hand_type in ['left', 'right']:
                        params = data_info['mano_params'][hand_type]
                    ...
                    masked_img = (torch.sum(img_mask, dim=-1) > 0.1).int().unsqueeze(-1).repeat(1,1,1,3).cpu() * img
                    img_mask = img_mask.detach().cpu().numpy()[0] * 255
                    masked_img = masked_img.detach().cpu().numpy()[0]

                    cv.imwrite(osp.join(save_path, split, 'img', str(capture_id), cam_id, '{}.jpg'.format(img_name)), img)
                    cv.imwrite(osp.join(save_path, split, 'mask', str(capture_id), cam_id, '{}.jpg'.format(img_name)), img_mask)
                    cv.imwrite(osp.join(save_path, split, 'masked_img', str(capture_id), cam_id, '{}.jpg'.format(img_name)), masked_img)
        elif target == 'single' and hand_type in ['left', 'right'] and hand_type_valid:
            mano_dict, coord_dict = loader.load_mano(i)
            if coord_dict[hand_type]:
                cam_id = images_info['camera']
                if cam_id.startswith('41') or cam_id in DROP_VIEW:
                    continue

                os.makedirs(osp.join(save_path, split, 'img', cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'mask', cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'masked_img', cam_id), exist_ok=True)
                os.makedirs(osp.join(save_path, split, 'anno', cam_id), exist_ok=True)
                
                hand = coord_dict[hand_type]['verts'][0].detach().numpy()
                img = loader.load_img(i)
                if img.mean() < 10:
                    continue

                cam_R, cam_t, cameraIn = loader.load_camera(i)
                hand = hand @ cam_R.T + cam_t
                hand2d = hand @ cameraIn.T
                hand2d = hand2d[:, :2] / hand2d[:, 2:]
        
                [img], _, cameraIn = \
                    cut_img([img], [hand2d], camera=cameraIn, radio=HAND_BBOX_RATIO, img_size=IMG_SIZE)
                cv.imwrite(osp.join(save_path, split, 'img', cam_id, '{}.jpg'.format(img_name)), img)
                ...
                data_info['mano_params'] = mano_dict
                data_info['camera'] = {'R': cam_R, 't': cam_t, 'camera': cameraIn}
                with open(osp.join(save_path, split, 'anno', cam_id, '{}.pkl'.format(img_name)), 'wb') as file:
                    pickle.dump(data_info, file)
                ...
                verts = torch.from_numpy(handV).float().cuda().unsqueeze(0)

                img_mask = renderer.render_mask(cameras=torch.from_numpy(camera).float().cuda().unsqueeze(0),
                                                v3d=verts)
                img_mask = img_mask.detach().cpu().numpy()[0] * 255
                white_mask = torch.sum(img_mask, dim=1)
                cv.imwrite(osp.join(save_path, split, 'mask', cam_id, '{}.jpg'.format(img_name)), img_mask)
                cv.imwrite(osp.join(save_path, split, 'masked_img', cam_id, '{}.jpg'.format(img_name)), img_mask * None)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default='/workspace/datasets/InterHand/')
    parser.add_argument("--save_path", type=str, default='/workspace/datasets/BiTT_InterHand_id/')
    parser.add_argument("--hand_type", type=str, default='both') # single
