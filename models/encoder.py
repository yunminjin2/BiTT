...
def load_encoder(cfg):
    if cfg.INTAGHANDMODEL.ENCODER_TYPE.find('resnet') != -1:
        encoder = ResNetSimple(model_type=cfg.INTAGHANDMODEL.ENCODER_TYPE,
                               pretrained=True,
                               fmapDim=[128, 128, 128, 128],
                               handNum=2,
                               heatmapDim=21)
        mid_model = resnet_mid(model_type=cfg.INTAGHANDMODEL.ENCODER_TYPE,
                               in_fmapDim=[128, 128, 128, 128],
                               out_fmapDim=cfg.INTAGHANDMODEL.DECONV_DIMS)
    if cfg.INTAGHANDMODEL.ENCODER_TYPE.find('hrnet') != -1:
        encoder = HRnet_encoder(model_type=cfg.INTAGHANDMODEL.ENCODER_TYPE,
                                pretrained=cfg.INTAGHANDMODEL.ENCODER_PRETRAIN_PATH,
                                handNum=2,
                                heatmapDim=21)
        mid_model = hrnet_mid(model_type=cfg.INTAGHANDMODEL.ENCODER_TYPE,
